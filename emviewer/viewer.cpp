...
public:
    ...
    void setWebView(IWebView2Environment* env) noexcept
    {
        m_env = env;
...
int CALLBACK wWinMain(HINSTANCE, HINSTANCE, LPWSTR commandLine, int show)
{
    ...
    HRESULT hr = CreateWebView2EnvironmentWithDetails(nullptr, wide(EMVIEWER_APP.data()), commandLine, Callback<IWebView2CreateWebView2EnvironmentCompletedHandler>(
        [show](HRESULT result, IWebView2Environment* env) {
            if (FAILED(result)) return result;

            WindowProgram::registerClass(0);
            g_mainwindow.open(show);
            g_mainwindow.setWebView(env);
            return S_OK;
        }).Get());
    if (FAILED(hr))
    {
        errorBox(TSZ16() << u"Cannot run WebView2, It needs Chromium Edge\nHRESULT: 0x" << hexf((uint)hr, 8));
        return hr;
    }
